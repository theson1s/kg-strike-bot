...
async def strike(ctx, user: discord.User, *, reason="No reason provided"):
    if not ctx.author.guild_permissions.administrator:
        await ctx.send("You need administrator permissions to give strikes.")
        return
    global strikes
    strikes = load_strikes()
    user_id = str(user.id)
...
class StrikeView(View):
    ...
    @discord.ui.button(label="Remove 1 Strike", style=discord.ButtonStyle.danger)
    async def remove_strike_button(self, interaction: discord.Interaction, button: Button):
        ...
        if self.user_id not in self.strikes_data or self.strikes_data[self.user_id]['count'] == 0:
            await interaction.response.send_message(f"{self.user.mention} has no strikes to remove.", ephemeral=True)
    ...
    @discord.ui.button(label="Remove ALL Strikes", style=discord.ButtonStyle.danger)
...
async def strikemenu(ctx, user: discord.User):
    if not ctx.author.guild_permissions.administrator:
        await ctx.send("You need administrator permissions to use this menu.")
        return
    user_id = str(user.id)
...
@bot.tree.command(name="mute", description="Mute a user (assigns Muted role)")
...
@bot.tree.command(name="unmute", description="Unmute a user (remove Muted role)")
@app_commands.checks.has_permissions(manage_roles=True)
...
async def slash_unmute(interaction: discord.Interaction, user: discord.User, reason: str = "No reason provided"):
    ...
    if not muted_role or muted_role not in member.roles:
        await interaction.followup.send(embed=discord.Embed(title="Not Muted", description=f"{user.mention} is not muted.", color=discord.Color.blue()))
        return
    try:
